...
#pragma endregion
	...
	{
		if (fMultiplier)
			m_qOrientation *= glm::angleAxis(glm::radians(1.0f), AXIS_X);
		else
			m_qOrientation *= glm::angleAxis(glm::radians(-1.0f), AXIS_X);
	}
	...
	{
		if (fMultiplier)
			m_qOrientation *= glm::angleAxis(glm::radians(1.0f), AXIS_Y);
		else
			m_qOrientation *= glm::angleAxis(glm::radians(-1.0f), AXIS_Y);
	}
	...
	{
		if (fMultiplier)
			m_qOrientation *= glm::angleAxis(glm::radians(1.0f), AXIS_Z);
		else
			m_qOrientation *= glm::angleAxis(glm::radians(-1.0f), AXIS_Z);
	}
